...
local in_siunitx = make_condition(tex.in_siunitx)
local in_math = make_condition(tex.in_math)
...
local M = {
    ...
    { condition = in_bullets * line_begin, show_condition = in_bullets * tex.show_line_begin }
    ),
    ...
    s({ trig = "eqn", name = "equation(|*)", dscr = "equation math" },
    fmta([[
    \begin{equation<>}<>
    <>
    .\end{equation<>}
    ...
    -- sub super scripts
    autosnippet({ trig = "([A-Za-z][)\\]}]?)(\\d)", trigEngine="ecma", name = "auto subscript", dscr = "auto subscript 1" },
    fmta([[
    <>_<>
    ]],
    ...
    autosnippet({ trig = "(\\[A-Za-z]+[)\\]}]?)(\\d)", trigEngine="ecma", name = "auto subscript 1.5", dscr = "auto subscript with symbols" },
    fmta([[
    <>_<>
    ]],
    { f(get_capture, {}, { user_args = { 1 } }), f(get_capture, {}, { user_args = { 2 } }) }),
    ...
    autosnippet({ trig = "sr", wordTrig = false }, { t("^2") }, { condition=in_math - in_siunitx }),
    autosnippet({ trig = "cb", wordTrig = false }, { t("^3") }, { condition=in_math - in_siunitx }),
    ...
    autosnippet({ trig='df([fsc])', trigEngine="pattern", name='diff', dscr='differential'},
    fmta([[
    \dif<><>{<>}{<>}
    ]],
    ...
    i(1), i(2) }),
    { condition = tex.in_math }),
    autosnippet({ trig='dfp([fsc])', trigEngine="pattern", name='diff', dscr='differential'},
    fmta([[
    \dif<>p<>{<>}{<>}
    ]],
    { f(get_capture, {}, { user_args = { 1 } }),
    ...
    autosnippet({ trig = "(?<!\\\\)lim", trigEngine = "ecma", name = "lim(sup|inf)", dscr = "lim(sup|inf)", hidden = true },
    fmta([[ 
    \lim<><><>
    ]],
    {c(1, { t(""), t("sup"), t("inf") }),
    ...
    autosnippet({ trig = "(%d)int", name = "multi integrals", dscr = "please work", trigEngine="pattern", hidden = true },
    fmta([[ 
    <> <> <> <>
    ]],
    { c(1, {
    ...
    -- siunitx: SI units
    autosnippet({ trig='siu', name='SI units', dscr='siunitx: SI units'},
    fmta([[
    \SI{<>}{<>}
    ]],
    { i(1), i(2) }
    )),
    autosnippet("sitest", { t("this works in siunitx") }, { condition=tex.in_siunitx }),
}
...
-- what
-- ]
local command_snippet = require("snippets.tex.utils").scaffolding.command_snippet

local command_specs = {
    ndc = {
        context = {
            name = "NewDocumentCommand",
            dscr = "LaTeX NewDocumentCommand",
        },
    ...
    rdc = {
        context = {
            ...
            dscr = "LaTeX RenewDocumentCommand",
        },
    ...
    tblk = {
        context = {
            name = "nicematrix Block",
            dscr = "nicematrix Block",
        },
        command = [[\Block]],
        argspec = {
            mandatory = 2,
...
local command_snippets = {}
for k, v in pairs(command_specs) do
    table.insert(
        command_snippets,
        command_snippet(
            vim.tbl_deep_extend("keep", { trig = k }, v.context),
...
local env_specs = {
    ...
    ans = {
        context = {
            name = "answer",
            dscr = "answer environment for hw"
        },
        command = [[answer]],
    },
    sol = {
        context = {
            name = "solution",
            dscr = "solution environment for hw",
        },
        command = [[solution]],
    }
}
...
local symbol_specs = {
    ...
    cc = { context = { name = "⊂", priority = 500}, command = [[\subset]] },
    cq = { context = { name = "⊆", priority = 500 }, command = [[\subseteq]] },
    scc = { context = { name = "⊂" }, command = [[\sqsubset]] },
    scq = { context = { name = "⊆" }, command = [[\sqsubseteq]] },
    snq = { context = { name = "⊆" }, command = [[\subsetneq]] },
    ...
    -- differentials 
    dd = { context = { name = "⇒" }, command = [[\dd]] },
    dp = { context = { name = "⇒" }, command = [[\ddp]] },
    -- arrows
    ...
    tri = { context = { name = "△" }, command = [[\triangle]] },
    tlq = { context = { name = "△" }, command = [[\trianglelefteq]] },
    trq = { context = { name = "△" }, command = [[\trianglerighteq]] },
}
...
local siunitx_specs = {
    jj =  { context = { name = "SI: joule" }, command = [[\joule]] },
    ["/"] =  { context = { name = "SI: per" }, command = [[\per]] },
    kg =  { context = { name = "SI: kilogram" }, command = [[\kilogram]] },
    tk =  { context = { name = "SI: Kelvin" }, command = [[\kelvin]] },
    nt =  { context = { name = "SI: newton" }, command = [[\newton]] },
    mt =  { context = { name = "SI: meter" }, command = [[\meter]] },
    sr =  { context = { name = "SI: squared" }, command = [[\squared]] },
    rs =  { context = { name = "SI: square" }, command = [[\square]] },
    cb =  { context = { name = "SI: cubed" }, command = [[\cubed]] },
    bc =  { context = { name = "SI: cubic" }, command = [[\cubic]] },
}

local siunitx_snippets = {}
for k, v in pairs(siunitx_specs) do
    table.insert(
        siunitx_snippets,
        symbol_snippet(vim.tbl_deep_extend("keep", { trig = k }, v.context), v.command, { condition = tex.in_siunitx })
    )
end
vim.list_extend(M, siunitx_snippets)
